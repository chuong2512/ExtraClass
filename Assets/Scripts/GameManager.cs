...
{
	[SerializeField] private Text idleCashText;

	[SerializeField] private Text mainScreenCash;

	[SerializeField] private Text shopScreenCash;

	[SerializeField] private Text mainScreenDiamond;

	[SerializeField] private Text shopScreenDiamond;

	[SerializeField] private GameObject kitchenPrefab;

	[SerializeField] private Transform kitchenContent;
	...
	[HideInInspector] public List<KitchenController> kitchenController;
	...
	private void Start()
	{
		...
		this.idleCashText,
		GameUtilities.Currencies.Convert(this.database.restaurant[this.database.targetRestaurant].idleCash)+"/s");
		this.InitKitchen();
		this.InitElevator();
		this.InitRestaurant();
		this.InitManager();
				...
				if(location!=Location.Elevator)
				{
					if(location!=Location.Restaurant)
					{
						if(location==Location.Kitchen)
						{
							...
							this.kitchenController[kitchenFloor].managerController.managerProfile=
								this.database.restaurant[this.database.targetRestaurant].profile[i];
	...
	public static bool IsDoneTutorial(int index) { return Singleton<DataManager>.Instance.database.tutorialCompleted.Contains(index); }

	public static void TutorialDone(int index) { Singleton<DataManager>.Instance.database.tutorialCompleted.Add(index); }
}
