	...
	private sealed class _Processing_c__Iterator0 : IEnumerator,IDisposable,IEnumerator<object>
	{
		...
		public bool MoveNext()
		{
			...
			if(this._this.barrierData.unlockRemaining>0)
			{
				...
				this._this.process.diamondText,
				((int) Math.Ceiling(
				(double) ((float) this._this.config.barrier[this._kitchenCount___0/this._this.config.kitchen.barrierStep-1].diamondToUnlock*
		...
		else if(flag && !this.general.barrierEnabled.activeInHierarchy)
		{
			...
			if(state!=BarrierState.Locked)
			{
				...
				else
				{
					...
					GameUtilities.String.ToText(
					this.process.reduceTimeText,
					"-"+GameUtilities.DateTime.Convert(this.config.barrier[count/this.config.kitchen.barrierStep-1].reduceProcessTime));
			...
			else
			{
				...
				GameUtilities.String.ToText(
				this.locked.processTimeText,
				GameUtilities.DateTime.Convert(this.config.barrier[count/this.config.kitchen.barrierStep-1].unlockDuration));
