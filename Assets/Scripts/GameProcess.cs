...
public class GameProcess : Singleton<GameProcess>
{
	private sealed class _GetManagerTextColor_c__AnonStorey0
	{
		...
		internal bool __m__0(Configuration.ExperienceRate target) { return target.experience==this.experience; }
	}
	...
	private sealed class _GetManagerSkillDuration_c__AnonStorey2
	{
		...
		internal bool __m__0(Configuration.SkillEffective target) { return target.experience==this.experience && target.skill==this.skill; }
	}
	...
	private sealed class _GetManagerSkillSprite_c__AnonStorey6
	{
		...
		internal bool __m__0(Configuration.SkillRate target) { return target.skill==this.skill; }
	}
	...
	{
		return Array.Find<Configuration.ExperienceRate>(
		this.configuration.experienceRate,
		(Configuration.ExperienceRate target) => target.experience==experience).textColor;
	}
	...
	public Color GetManagerBorderColor(Experience experience)
	{
		return Array.Find<Configuration.ExperienceRate>(
		this.configuration.experienceRate,
		(Configuration.ExperienceRate target) => target.experience==experience).borderColor;
	}
	...
	public ManagerProfile GetManagerProfile(Location location,double price,bool noJunior)
	{
		...
		while (true)
		{
			...
			if((location!=Location.Elevator || managerProfile.skill!=ManagerSkill.CookingSpeed) &&
			   (location!=Location.Elevator || managerProfile.skill!=ManagerSkill.WalkingSpeed))
			{
				if((location!=Location.Restaurant || managerProfile.skill!=ManagerSkill.CookingSpeed) &&
				   (location!=Location.Restaurant || managerProfile.skill!=ManagerSkill.MovementSpeed))
				{
					if((location!=Location.Kitchen || managerProfile.skill!=ManagerSkill.LoadExpansion) &&
					   (location!=Location.Kitchen || managerProfile.skill!=ManagerSkill.MovementSpeed) &&
					   (location!=Location.Kitchen || managerProfile.skill!=ManagerSkill.LoadingSpeed))
					{
						break;
					}
				}
	...
	public double GetManagerPrice(int count,Location position)
	{
		...
		{
			return Math.Round(
			(double) this.configuration.kitchen.baseManagerPrice*Math.Pow((double) this.configuration.kitchen.hireManagerFactor,(double) count));
		}
		if(position==Location.Elevator)
		{
			return Math.Round(
			(double) this.configuration.elevator.baseManagerPrice*Math.Pow((double) this.configuration.elevator.hireManagerFactor,(double) count));
		}
		if(position==Location.Restaurant)
		{
			return Math.Round(
			(double) this.configuration.restaurant.baseManagerPrice*Math.Pow(
			(double) this.configuration.restaurant.hireManagerFactor,
			(double) count));
		}
		return 0.0;
	}
	...
	{
		return Array.Find<Configuration.SkillEffective>(
		this.configuration.skillEffective,
		(Configuration.SkillEffective target) => target.experience==experience && target.skill==skill).remaining;
	}
	...
	public int GetManagerSkillCooldown(Experience experience,ManagerSkill skill)
	{
		return Array.Find<Configuration.SkillEffective>(
		this.configuration.skillEffective,
		(Configuration.SkillEffective target) => target.experience==experience && target.skill==skill).cooldown;
	}
	...
	public int GetManagerSkillEffective(Experience experience,ManagerSkill skill)
	{
		return Array.Find<Configuration.SkillEffective>(
		this.configuration.skillEffective,
		(Configuration.SkillEffective target) => target.experience==experience && target.skill==skill).effective;
	}
	...
	public Sprite GetManagerAvatarSprite(Experience experience)
	{
		return Array.Find<Configuration.ExperienceRate>(
		this.configuration.experienceRate,
		(Configuration.ExperienceRate target) => target.experience==experience).avatar;
	}

	public Sprite GetManagerSkillSprite(ManagerSkill skill,bool value)
	{
		Configuration.SkillRate skillRate=Array.Find<Configuration.SkillRate>(
		this.configuration.skillRate,
		(Configuration.SkillRate target) => target.skill==skill);
	...
	public double GetFloorPrice(int floor)
	{
		...
		return Math.Round(
		(double) this.configuration.kitchen.baseUnlockPrice*Math.Pow((double) this.configuration.kitchen.unlockPriceFactor,(double) floor)*
		Math.Pow((double) unlockBarrierFactor,(double) (floor/barrierStep)));
	}
	...
	public KitchenProperties GetKitchenProperties(float distance,int floor,int level,float multiplier)
	{
		...
		double            floorPrice=this.GetFloorPrice(floor);
		double num=(double) this.configuration.kitchen.transporterCapacityFactor/Math.Pow(
		(double) this.configuration.kitchen.transporterCapacityFactor,
		(double) floor);
	...
	public ElevatorProperties GetElevatorProperties(float distance,int level)
	{
		...
		result.movementSpeed=(float) Math.Round(
		(double) (this.configuration.elevator.baseMovementSpeed+
		          (float) level*(this.configuration.elevator.baseMovementSpeed*(this.configuration.elevator.movementSpeedFactor/100f))),
		2);
