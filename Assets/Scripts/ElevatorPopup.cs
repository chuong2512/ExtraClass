...
public class ElevatorPopup : MonoBehaviour
{
	...
	[SerializeField] private Text currentTotal;

	[SerializeField] private Text nextTotal;

	[SerializeField] private Text currentLoad;

	[SerializeField] private Text nextLoad;

	[SerializeField] private Text currentLoading;

	[SerializeField] private Text nextLoading;

	[SerializeField] private Text currentSpeed;

	[SerializeField] private Text nextSpeed;

	[SerializeField] private Image currentLevelFill;

	[SerializeField] private Image nextLevelFill;

	[SerializeField] private Text bonusDiamond;
	...
	public void Show()
	{
		...
		this.nextLevel,
		(nextBonusAtLevel!=2147483647) ? ("Next boost at level "+nextBonusAtLevel.ToString()) : "Boost maximum.");
		this.currentLevelFill.fillAmount=((nextBonusAtLevel!=2147483647)
			? ((float) (this.elevatorController.elevatorData.level-lastBonusAtLevel)/(float) (nextBonusAtLevel-lastBonusAtLevel))
			: 1f);
		...
		else
		{
			...
			{
				num=Singleton<GameProcess>.Instance.GetMaxUpgradeLevel(
				Singleton<GameManager>.Instance.database.cash,
				this.elevatorController.boostController.upgradeCostReduced,
				this.elevatorController.elevatorData.level,
				Location.Elevator,
				0);
			}
			...
			ElevatorProperties elevatorProperties2=
				Singleton<GameProcess>.Instance.GetElevatorProperties(distance,this.elevatorController.elevatorData.level+num);
			...
			GameUtilities.String.ToText(
			this.nextSpeed,
			"+"+Math.Round((double) (elevatorProperties2.movementSpeed-elevatorProperties.movementSpeed),2).ToString());
			GameUtilities.String.ToText(
			this.nextLoading,
			"+"+GameUtilities.Currencies.Convert(elevatorProperties2.loadingSpeed-elevatorProperties.loadingSpeed)+"/s");
			GameUtilities.String.ToText(
			this.nextTotal,
			"+"+GameUtilities.Currencies.Convert(elevatorProperties2.transportation-elevatorProperties.transportation)+"/s");
			double num2=Singleton<GameProcess>.Instance.GetUpgradePrice(
			this.elevatorController.elevatorData.level,
			num,
			this.elevatorController.boostController.upgradeCostReduced,
			Location.Elevator,
			...
			this.upgradeButton.sprite=
				((Singleton<GameManager>.Instance.database.cash<num2 || this.elevatorController.elevatorData.level+num>
					Singleton<GameProcess>.Instance.GetMaxLevel(Location.Elevator))
					? this.disableSprite
					: this.enableSprite);
			this.nextLevelFill.fillAmount=((nextBonusAtLevel!=2147483647)
				? ((float) (this.elevatorController.elevatorData.level+num-lastBonusAtLevel)/(float) (nextBonusAtLevel-lastBonusAtLevel))
				: 1f);
			int diamondBonus=Singleton<GameProcess>.Instance.GetDiamondBonus(
