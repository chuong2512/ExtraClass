...
public class CoinItemPool : MonoBehaviour
{
	private sealed class _Spawn_c__Iterator0 : IEnumerator,IDisposable,IEnumerator<object>
	{
		...
		public bool MoveNext()
		{
			...
			{
				this._x___1=UnityEngine.Random.Range(
				this._this.transform.position.x-this._this.randomRadius,
				this._this.transform.position.x+this._this.randomRadius);
				this._y___1=UnityEngine.Random.Range(
				this._this.transform.position.y-this._this.randomRadius,
				this._this.transform.position.y+this._this.randomRadius);
				...
				this._coinItem___1.GetComponent<CoinItem>().Init(
				new Vector3[]
				{
	...
	private void Start() { this.waitForSeconds=new WaitForSeconds(this.spawnDeltaTime); }
